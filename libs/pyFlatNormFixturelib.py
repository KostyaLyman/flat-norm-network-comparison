...
from pyDrawNetworklib import plot_regions, plot_triangulation, plot_hausdorff
from pyHausdorfflib import compute_hausdorff
...
class FlatNormFixture(unittest.TestCase):
    ...
    def compute_region_hausdorff(self, region, act_geom, synt_geom):
        # get the actual and synthetic geometries within region
        reg_act_geom = [g for g in act_geom if g.intersects(region)]
        reg_synt_geom = [g for g in synt_geom if g.intersects(region)]
        
        # compute hausdorff distance between the geometries
        return compute_hausdorff(reg_act_geom, reg_synt_geom)
    ...
    def compute_region_flatnorm(
        ...
        hdtup = None
        if compare_hausdorff:
            hdtup = self.compute_region_hausdorff(region, act_geom, synt_geom)

        # --- normalization of flat norm ---
    ...
    def plot_triangulated_region_flatnorm(
        ...
        do_return = kwargs.get('do_return', False)
        if haus:
            figsize = kwargs.get('figsize', (25, 10))
            fig, axs, no_ax = get_fig_from_ax(ax, figsize, ndim=(1, 3))
            plot_hausdorff(triangulated, T1, T2, haus[1], axs[2])
            suffix = r" : $d_{Hausdorff}$" + f" = {haus[0]:0.5f}"
        else:
        ...
        if no_ax:
            to_file = f"{self.fig_dir}/{to_file}.png"
            if kwargs.get('suptitle_sfx'):
                suptitle = f"{kwargs.get('suptitle_sfx')}" + suffix
