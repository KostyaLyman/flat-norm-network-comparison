...
def compute_flatnorm_region(point, geom1, geom2,
                            ...
                            normalized=False):
    # Get the region
    region = point.buffer(epsilon, cap_style=CAP_STYLE.square)
    ...
    x, s, norm, norm1, norm2, w = msfn(D['triangulated']['vertices'],
...
#     for lamb in [1000, 100000]:
for eps in np.linspace(5e-4, 2e-3, 4):
    for lamb in np.linspace(1000, 100000, 5):
        start = timer()
...
ax = fig.add_subplot(111)
ax = sns.lineplot(data=plot_data_df, x='lambda', y='flatnorm', hue='epsilon', ax=ax,
